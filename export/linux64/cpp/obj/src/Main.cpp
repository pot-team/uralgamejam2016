...
Void Main_obj::loadConfig( ){
{
		HX_STACK_FRAME("Main","loadConfig",0x1485037d,"Main.loadConfig","Main.hx",44,0x087e5c05)
		HX_STACK_THIS(this)
		HX_STACK_LINE(45)
		::flixel::util::FlxSave tmp = ::flixel::util::FlxSave_obj::__new();		HX_STACK_VAR(tmp,"tmp");
		HX_STACK_LINE(45)
		::flixel::util::FlxSave _settings = tmp;		HX_STACK_VAR(_settings,"_settings");
		HX_STACK_LINE(46)
		_settings->bind(HX_HCSTRING("gamesettings","\xd5","\xb2","\x7c","\xec"));
		HX_STACK_LINE(47)
		bool tmp1 = (_settings->data->__Field(HX_HCSTRING("is_existed","\x41","\xa6","\xef","\xfe"), hx::paccDynamic ) != null());		HX_STACK_VAR(tmp1,"tmp1");
		HX_STACK_LINE(47)
		if ((tmp1)){
			HX_STACK_LINE(49)
			this->gameWidth = _settings->data->__Field(HX_HCSTRING("gameWidth","\xb4","\xa4","\x7d","\xff"), hx::paccDynamic );
			HX_STACK_LINE(50)
			this->gameHeight = _settings->data->__Field(HX_HCSTRING("gameHeight","\x79","\xf1","\xc1","\x44"), hx::paccDynamic );
			HX_STACK_LINE(51)
			this->startFullscreen = _settings->data->__Field(HX_HCSTRING("startFullscreen","\x3d","\xc1","\xee","\x2d"), hx::paccDynamic );
		}
		else{
			HX_STACK_LINE(59)
			int tmp2 = this->gameWidth;		HX_STACK_VAR(tmp2,"tmp2");
			HX_STACK_LINE(59)
			_settings->data->__FieldRef(HX_HCSTRING("gameWidth","\xb4","\xa4","\x7d","\xff")) = tmp2;
			HX_STACK_LINE(60)
			int tmp3 = this->gameHeight;		HX_STACK_VAR(tmp3,"tmp3");
			HX_STACK_LINE(60)
			_settings->data->__FieldRef(HX_HCSTRING("gameHeight","\x79","\xf1","\xc1","\x44")) = tmp3;
			HX_STACK_LINE(61)
			bool tmp4 = this->startFullscreen;		HX_STACK_VAR(tmp4,"tmp4");
			HX_STACK_LINE(61)
			_settings->data->__FieldRef(HX_HCSTRING("startFullscreen","\x3d","\xc1","\xee","\x2d")) = tmp4;
			HX_STACK_LINE(62)
			_settings->data->__FieldRef(HX_HCSTRING("is_existed","\x41","\xa6","\xef","\xfe")) = true;
			HX_STACK_LINE(63)
			_settings->flush(null(),null());
		}
	}
return null();
}
...
HX_DEFINE_DYNAMIC_FUNC0(Main_obj,loadConfig,(void))

Void Main_obj::init( ::openfl::_legacy::events::Event E){
{
		HX_STACK_FRAME("Main","init",0xea732345,"Main.init","Main.hx",68,0x087e5c05)
		HX_STACK_THIS(this)
		HX_STACK_ARG(E,"E")
		HX_STACK_LINE(69)
...
{
		HX_STACK_FRAME("Main","setupGame",0x7f7688ba,"Main.setupGame","Main.hx",78,0x087e5c05)
		HX_STACK_THIS(this)
		HX_STACK_LINE(79)
...
Dynamic Main_obj::__Field(const ::String &inName,hx::PropertyAccess inCallProp)
{
	...
	case 10:
		...
		if (HX_FIELD_EQ(inName,"loadConfig") ) { return loadConfig_dyn(); }
		break;
	case 12:
...
static ::String sMemberFields[] = {
	...
	HX_HCSTRING("loadConfig","\x88","\x85","\xfd","\x7c"),
